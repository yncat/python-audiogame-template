...
bass_fx_module = libloader.load_library(
...
class BASS_BFX_MIX(ctypes.Structure):
    _fields_ = [
        # an array of channels to mix using BASS_BFX_CHANxxx flag/s
        # (lChannel[0] is left channel...)
        ('lChannel', ctypes.c_int),
    ]
...
class BASS_BFX_DAMP(ctypes.Structure):
    _fields_ = [
        ...
        ('fTarget', ctypes.c_float),
        # quiet  volume level						[0.......1] linear
        ('fQuiet', ctypes.c_float),
        # amp adjustment rate						[0.......1] linear
        ('fRate', ctypes.c_float),
...
class BASS_BFX_AUTOWAH(ctypes.Structure):
    _fields_ = [
        ...
        ('fDryMix', ctypes.c_float),
        # wet (affected) signal mix				[-2......2]
        ('fWetMix', ctypes.c_float),
...
BASS_FX_TempoCreate = func_type(
    pybass.HSTREAM, ctypes.c_ulong, ctypes.c_ulong)(
        ('BASS_FX_TempoCreate', bass_fx_module))

# DWORD BASS_FXDEF(BASS_FX_TempoGetSource)(HSTREAM chan);
BASS_FX_TempoGetSource = func_type(
    ctypes.c_ulong, pybass.HSTREAM)(
        ('BASS_FX_TempoGetSource', bass_fx_module))

# float BASS_FXDEF(BASS_FX_TempoGetRateRatio)(HSTREAM chan);
BASS_FX_TempoGetRateRatio = func_type(
    ctypes.c_float, pybass.HSTREAM)(
        ('BASS_FX_TempoGetRateRatio', bass_fx_module))
...
# HSTREAM BASS_FXDEF(BASS_FX_ReverseCreate)(DWORD chan, float dec_block,
# DWORD flags);
BASS_FX_ReverseCreate = func_type(
    pybass.HSTREAM, ctypes.c_ulong, ctypes.c_float, ctypes.c_ulong)(
        ('BASS_FX_ReverseCreate', bass_fx_module))

# DWORD BASS_FXDEF(BASS_FX_ReverseGetSource)(HSTREAM chan);
BASS_FX_ReverseGetSource = func_type(
    ctypes.c_ulong, pybass.HSTREAM)(
        ('BASS_FX_ReverseGetSource', bass_fx_module))

"""B P M (Beats Per Minute)"""

# bpm flags
# if in use, then you can do other processing while detection's in
# progress. Not available in MacOSX yet. (BPM/Beat)
...
# translation options
(
    # multiply the original BPM value by 2 (may be called only once & will
    # change the original BPM as well!)
    BASS_FX_BPM_TRAN_X2,
...
# float BASS_FXDEF(BASS_FX_BPM_DecodeGet)(DWORD chan, double startSec,
# double endSec, DWORD minMaxBPM, DWORD flags, BPMPROCESSPROC *proc);
...
# BOOL BASS_FXDEF(BASS_FX_BPM_CallbackSet)(DWORD handle, BPMPROC *proc,
# double period, DWORD minMaxBPM, DWORD flags, void *user);
...
BASS_FX_BPM_CallbackReset = func_type(
    ctypes.c_bool, ctypes.c_ulong)(
        ('BASS_FX_BPM_CallbackReset', bass_fx_module))

# float BASS_FXDEF(BASS_FX_BPM_Translate)(DWORD handle, float val2tran,
# DWORD trans);
BASS_FX_BPM_Translate = func_type(
    ctypes.c_float, ctypes.c_ulong, ctypes.c_float, ctypes.c_ulong)(
        ('BASS_FX_BPM_Translate', bass_fx_module))

# BOOL BASS_FXDEF(BASS_FX_BPM_Free)(DWORD handle);
BASS_FX_BPM_Free = func_type(
    ctypes.c_bool, ctypes.c_ulong)(
        ('BASS_FX_BPM_Free', bass_fx_module))
...
# BOOL BASS_FXDEF(BASS_FX_BPM_BeatCallbackSet)(DWORD handle, BPMBEATPROC
# *proc, void *user);
BASS_FX_BPM_BeatCallbackSet = func_type(
    ctypes.c_bool, ctypes.c_ulong, ctypes.c_void_p)(
        ('BASS_FX_BPM_BeatCallbackSet', bass_fx_module))

# BOOL BASS_FXDEF(BASS_FX_BPM_BeatCallbackReset)(DWORD handle);
BASS_FX_BPM_BeatCallbackReset = func_type(
    ctypes.c_bool, ctypes.c_ulong)(
        ('BASS_FX_BPM_BeatCallbackReset', bass_fx_module))

# BOOL BASS_FXDEF(BASS_FX_BPM_BeatDecodeGet)(DWORD chan, double startSec,
# double endSec, DWORD flags, BPMBEATPROC *proc, void *user);
...
# BOOL BASS_FXDEF(BASS_FX_BPM_BeatSetParameters)(DWORD handle, float
# bandwidth, float centerfreq, float beat_rtime);
...
# BOOL BASS_FXDEF(BASS_FX_BPM_BeatGetParameters)(DWORD handle, float
# *bandwidth, float *centerfreq, float *beat_rtime);
...
BASS_FX_BPM_BeatFree = func_type(
    ctypes.c_bool, ctypes.c_ulong)(
        ('BASS_FX_BPM_BeatFree', bass_fx_module))
