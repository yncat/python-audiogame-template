...
class Output (object):
    ...
    def init_device(
        ...
        if clsid is None:
            clsid = self.clsid
        self.clsid = clsid
        if platform.system() == 'Linux' and device == - \
                1:  # Bass wants default device set to 1 on linux
            device = 1
    ...
    def set_volume(self, volume):
        ...
        return bass_call(
            BASS_SetConfig, BASS_CONFIG_GVOL_STREAM, int(
                round(
                    volume * 100, 2)))
...
class ThreeDOutput(Output):
    ...
    def get_3d_factors(self):
        ...
        bass_call(
            BASS_Get3DFactors, pointer(
                res['distance_factor']), pointer(
                res['rolloff']), pointer(
                res['doppler_factor']))
